...
@kernel function compute_bld!(h, grid, b, u, v, w, Ric)
    ...
    @unroll for k in k_start : -1 : 1 # scroll from point just below surface
        ...
        Δb = b_ref - bᵏ
        Δu² = (u_ref - uᵏ)^2 + (v_ref - vᵏ)^2 + (w_ref - wᵏ)^2
        zᵏ = znode(i, j, k, grid, c, c, c)
        Riᵏ = Δu² > 0 ? zᵏ * Δb / Δu² : 0.0 # avoid division by zero
        Δb⁺ = b_ref - b⁺
        Δu²⁺ = (u_ref - u⁺)^2 + (v_ref - v⁺)^2 + (w_ref - w⁺)^2
        z⁺ = znode(i, j, k+1, grid, c, c, c)
        Ri⁺ = Δu²⁺ > 0 ? z⁺ * Δb⁺ / Δu²⁺ : 0.0 # avoid division by zero

        Δz⁺ = Δzᶜᶜᶠ(i, j, k+1, grid)

        # Assuming Richardson number decreases upwards
        ...
        if 0.1 * new_z_ij < z_ref
            new_z_ref = 0.1 * new_z_ij
...
struct BoundaryLayerDepthOperand{B, U, V, W, FT, G}
...
function BoundaryLayerDepth(grid, fields; Ric = 0.3, kw...)
    operand = BoundaryLayerDepthOperand(fields.b, fields.u, fields.v, fields.w, abs(Ric), grid)
...
function compute!(h::BoundaryLayerDepthField, time=nothing)
    ...
    launch!(arch, h.grid, :xy, compute_bld!, h, h.grid, b, u, v, w, Ric)
    fill_halo_regions!(h)
    return h
end