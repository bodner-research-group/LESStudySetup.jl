...
function compute_b3_fields(x, z, Ro, b0, ε, Fbs, Fvs, ψ¹ₚs, ϕ12s, t; ψ¹ₚγ = nothing, γ = 0.03)
    if isnothing(ψ¹ₚγ)
        ...
        b1, _ = compute_b1v1_field(x, z, Ro, γ/ε*Fbs[1].+Fbs[2].+Fbs[3], γ/ε*Fvs[1].+Fvs[2].+Fvs[3], γ/ε*ψ¹ₚγ .- ψ¹ₚs[1], ϕ12s, t)
        bVH = b0 .+ ε * b1
    end
...
ek_values = [1e-4, 1e-3, 1e-2, 1e-1]
ro_values = [0.25, 0.5, 1, 1.6]
γ_values = [0.0, 0.025, 0.05, 0.1]
d16_erf = ones(length(ek_values), length(ro_values))
...
# Define a list of markers to use for each Ro value
markers = [:circle, :utriangle, :diamond, :star5]
color4 = [:gray, :blue, :red, :black]
# Create figure and axis
figvff = Figure(size = (640, 320))
ax_b = Axis(figvff[1, 1]; xscale = log10,yscale = log10, ylabel = L"1/d", xlabel = L"Ro^2/Ek", limits = ((0.1,1e5),nothing), titlealign = :left, title = L"\text{(a) Front}")
ax_b2 = Axis(figvff[1, 2]; xscale = log10,yscale = log10, xlabel = L"Ro^2/Ek", limits = ((0.1,1e5),nothing), titlealign = :left, title = L"\text{(b) Filament}")
for (j, ro) in enumerate(ro_values)
    local ϕxx_erf, ϕxz_erf, ϕzz_erf = compute_ϕ12_field(x, z, ro, B_funcs_erf)
    local ϕxx_gauss, ϕxz_gauss, ϕzz_gauss = compute_ϕ12_field(x, z, ro, B_funcs_gauss)
    local ϕzzz_erf, ϕxzz_erf, ϕzxx_erf, ϕxxx_erf, Fγv_erf, Fγb_erf = compute_blt_fields(x, z, ro, B_funcs_erf)
    local ϕzzz_gauss, ϕxzz_gauss, ϕzxx_gauss, ϕxxx_gauss, Fγv_gauss, Fγb_gauss = compute_blt_fields(x, z, ro, B_funcs_gauss)
    local psi1_erf_orig = compute_psi1_field(x, z, ro, B_funcs_erf)
    local psi1_gauss_orig = compute_psi1_field(x, z, ro, B_funcs_gauss)
    local psi1_erf_new = compute_psi1_field(x, z, ro, B_funcs_erf; use_new_A_coeffs=true)
    local psi1_gauss_new = compute_psi1_field(x, z, ro, B_funcs_gauss; use_new_A_coeffs=true)
    local psi1s_erf = (psi1_erf_orig, psi1_erf_new)
    local psi1s_gauss = (psi1_gauss_orig, psi1_gauss_new)
    local Fbs_erf = (Fγb_erf, ϕzzz_erf, ϕzxx_erf)
    local Fvs_erf = (Fγv_erf, ϕxzz_erf, ϕxxx_erf)
    local Fbs_gauss = (Fγb_gauss, ϕzzz_gauss, ϕzxx_gauss)
    local Fvs_gauss = (Fγv_gauss, ϕxzz_gauss, ϕxxx_gauss)
    local psi1_erf_strain = compute_psi1_field(x, z, ro, B_funcs_erf; strain=true)
    local psi1_gauss_strain = compute_psi1_field(x, z, ro, B_funcs_gauss; strain=true)
    b0_erf, _, _ = compute_b3_fields(x, z, ro, b_erf, ek_values[1], Fbs_erf, Fvs_erf, psi1s_erf, (ϕxx_erf, ϕxz_erf, ϕzz_erf), 0.0)
    b0_gauss, _, _ = compute_b3_fields(x, z, ro, b_gauss, ek_values[1], Fbs_gauss, Fvs_gauss, psi1s_gauss, (ϕxx_gauss, ϕxz_gauss, ϕzz_gauss), 0.0)
    dbdx0 = maximum(abs, central_diff(b0_erf, dx, 1)[:,nz])
    dbdx0_gauss = maximum(abs, central_diff(b0_gauss, dx, 1)[:,nz])
    for (i, ek) in enumerate(ek_values)
        println("Ek = $ek, Ro = $ro")
        bt_erf, _, bVH_erf = compute_b3_fields(x, z, ro, b_erf, ek, Fbs_erf, Fvs_erf, psi1s_erf, (ϕxx_erf, ϕxz_erf, ϕzz_erf), 10.0)
        bt_gauss, _, bVH_gauss = compute_b3_fields(x, z, ro, b_gauss, ek, Fbs_gauss, Fvs_gauss, psi1s_gauss, (ϕxx_gauss, ϕxz_gauss, ϕzz_gauss), 10.0)

        dbdx_erf = central_diff(bt_erf, dx, 1)
        dbdx_gauss = central_diff(bt_gauss, dx, 1)
        d16_erf[i,j] = dbdx0/maximum(abs, dbdx_erf[:,nz])
        d16_gauss[i,j] = dbdx0_gauss/maximum(abs, dbdx_gauss[:,nz])

        for (k, γ) in enumerate(γ_values)
            bt_erf, _, bVH_erf = compute_b3_fields(x, z, ro, b_erf, ek, Fbs_erf, Fvs_erf, psi1s_erf, (ϕxx_erf, ϕxz_erf, ϕzz_erf), 10.0; ψ¹ₚγ=psi1_erf_strain, γ=γ)
            bt_gauss, _, bVH_gauss = compute_b3_fields(x, z, ro, b_gauss, ek, Fbs_gauss, Fvs_gauss, psi1s_gauss, (ϕxx_gauss, ϕxz_gauss, ϕzz_gauss), 10.0; ψ¹ₚγ=psi1_gauss_strain, γ=γ)
            ...
            if i==1 
                ...
                dbdx_erf = central_diff(bs_erf, dx, 1)
                dbdx_gauss = central_diff(bs_gauss, dx, 1)
                ds4_erf[j,k] = dbdx0/maximum(abs, dbdx_erf[:,nz])
                ds4_gauss[j,k] = dbdx0_gauss/maximum(abs, dbdx_gauss[:,nz])
            end

        end
    end
    # 1. Calculate X for this specific Ro (broadcasts against all Ek)
    x_data = (ro^2) ./ ek_values
    
    # 2. Extract Y for this specific Ro (the i-th column of the data)
    y_data_erf = 1.0 ./ d16_erf[:, j]
    y_data_gauss = 1.0 ./ d16_gauss[:, j]
    
    # 3. Plot with specific marker for this Ro and color by Ek
    # We use 'colormap' and 'colorrange' to ensure colors stay consistent across loops
    scatter!(ax_b, x_data, y_data_erf; 
        marker = markers[j], 
        color = color4, 
        colorrange = extrema(ek_values),
        label = "Ro = $ro" # Optional: adds label for legend
    )
    
    scatter!(ax_b2, x_data, y_data_gauss; 
        marker = markers[j], 
        color = color4,
        colorrange = extrema(ek_values)
    )
end
hlines!(ax_b, [1.0]; linestyle = :dash, color = :black)
hlines!(ax_b2, [1.0]; linestyle = :dash, color = :black)
# Add a legend to distinguish the markers
axislegend(ax_b; position = :lt)
# 2. Manual Legend for Ek (Colors) in ax_b2
# Create marker elements for the legend
ek_elements = [MarkerElement(color = c, marker = :circle, strokecolor = :transparent) for c in color4]
# Create labels
ek_labels = [L"Ek_V=10^{-4}", L"Ek_V=10^{-3}", L"Ek_V=10^{-2}", L"Ek_V=10^{-1}"]
ro_labels = [L"Ro=0.25", L"Ro=0.5", L"Ro=1.0", L"Ro=1.6"]

# Add the manual legend to ax_b2
axislegend(ax_b2, ek_elements, ek_labels; position = :lt)
resize_to_layout!(figvff)
save("ro2ek_ff.png", figvff)
save("ro2ek_ff.pdf", figvff; pt_per_unit = 1)
...
for (j, ro) in enumerate(ro_values)
    for (k, γ) in enumerate(γ_values)
        ax = axs[2*(k-1)+1]
        scatter!(ax, (ro^2) ./ ek_values, 1.0 ./ d64_erf[:, j, k]; 
            marker = markers[j], 
            color = color4, 
            colorrange = extrema(ek_values),
        ...
        ax = axs[2*(k-1)+2]
        scatter!(ax, (ro^2) ./ ek_values, 1.0 ./ d64_gauss[:, j, k]; 
            marker = markers[j], 
            color = color4,
            colorrange = extrema(ek_values)
        )
...
ax_e = Axis(g42[3, 1]; ylabel = L"1/d", title = L"\text{(e) Front }\varepsilon_S=0.05", ax_kwargs...)
ax_f = Axis(g42[3, 2]; title = L"\text{(f) Filament }\varepsilon_S=0.05", ax_kwargs...)
ax_g = Axis(g42[4, 1]; xlabel = L"Ro^2/Ek", ylabel = L"1/d", title = L"\text{(g) Front }\varepsilon_S=0.1", ax_kwargs...)
ax_h = Axis(g42[4, 2]; xlabel = L"Ro^2/Ek", title = L"\text{(h) Filament }\varepsilon_S=0.1", ax_kwargs...)
axs = [ax_a, ax_b, ax_c, ax_d, ax_e, ax_f, ax_g, ax_h]
hidexdecorations!(ax_a, ticks=false)
hidexdecorations!(ax_b, ticks=false)
hidexdecorations!(ax_c, ticks=false)
...
for (j, ro) in enumerate(ro_values)
    for (k, γ) in enumerate(γ_values)
        ax = axs[2*(k-1)+1]
        scatter!(ax, (ro^2) ./ ek_values, 1.0 ./ d64VH_erf[:, j, k]; 
            marker = markers[j], 
            color = color4, 
            colorrange = extrema(ek_values),
            label = ro_labels[j] # Optional: adds label for legend
        ...
        hlines!(ax, [1.0/ds4_erf[j,k]]; linestyle = :dash, color = :black)
        ax = axs[2*(k-1)+2]
        scatter!(ax, (ro^2) ./ ek_values, 1.0 ./ d64VH_gauss[:, j, k]; 
            marker = markers[j], 
            color = color4,
            colorrange = extrema(ek_values)
        )
...
rowgap!(g42, 3)
resize_to_layout!(figvff)
save("ro2ek_sff.png", figsvff)
save("ro2ek_sff.pdf", figsvff; pt_per_unit = 1)
